...
<div class="min-h-screen bg-background text-foreground">
	...
	<main class="max-w-6xl mx-auto p-6">
		...
		<div class="flex gap-2 mb-6">
			<button 
				class="px-4 py-2 border-2 border-border font-heading hover:translate-x-1 hover:translate-y-1 transition-transform {activeTab === 'nodes' ? 'bg-main text-main-foreground shadow-shadow' : 'bg-background text-foreground'}"
				on:click={() => switchTab('nodes')}
			>
				nodes
			</button>
			<button 
				class="px-4 py-2 border-2 border-border font-heading hover:translate-x-1 hover:translate-y-1 transition-transform {activeTab === 'users' ? 'bg-main text-main-foreground shadow-shadow' : 'bg-background text-foreground'}"
			...
			<div class="bg-secondary-background border-2 border-border p-6 shadow-shadow">
				...
				{#if nodes.length}
					<div class="grid gap-4">
						{#each nodes as node}
							<div class="bg-background border-2 border-border p-4 shadow-shadow">
								<div class="flex items-center justify-between">
									...
									<div class="flex items-center gap-3">
										<div class="text-right text-sm">
											<div class="px-2 py-1 border-2 border-border text-xs font-heading {node.is_online ? 'bg-chart-4 text-main-foreground' : 'bg-chart-1 text-main-foreground'}">
				...
				{:else}
					<div class="text-center py-12">
						<div class="w-20 h-20 mx-auto mb-4 bg-foreground/10 border-2 border-border flex items-center justify-center">
						...
						<p class="text-foreground/70 mb-6">add compute nodes to start hosting containers</p>
					</div>
				{/if}
			</div>
		{/if}
							...
							<div class="bg-background border-2 border-border p-4 shadow-shadow">
								<div class="flex items-center justify-between">
									...
									<div class="flex items-center gap-3">
										<div class="text-right text-sm">
											{#if user.is_admin}
												<div class="px-2 py-1 bg-chart-3 text-main-foreground border-2 border-border text-xs font-heading">admin</div>
											{/if}
											{#if user.container_id}
												<div class="px-2 py-1 bg-chart-4 text-main-foreground border-2 border-border text-xs font-heading mt-1">has container</div>
											{/if}
										</div>
...
<Modal show={showNodeModal} title="Add Node" onClose={() => showNodeModal = false}>
...
<Modal show={showTokenModal} title="Node Token" onClose={() => showTokenModal = false}>
	<div class="space-y-4">
		<p class="text-foreground/70">Copy this token and use it to authenticate the node:</p>
		<div class="bg-background border-2 border-border p-4 font-mono text-sm break-all">
			{currentToken}
		</div>
	...
	<div slot="footer" class="flex gap-3">
		<button 
			class="bg-main text-main-foreground border-2 border-border px-4 py-2 font-heading hover:translate-x-1 hover:translate-y-1 transition-transform shadow-shadow"
			on:click={() => {navigator.clipboard.writeText(currentToken); toastContainer.addToast('Token copied!', 'success')}}
		>
			copy token
		</button>
		<button 
